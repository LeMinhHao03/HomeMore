...
<title>Quản Lý Tài Khoản | Admin Home&More</title>

<link href="/HomeMore/Admin/assets/css/bootstrap.css"
	rel="stylesheet" />
<link href="/HomeMore/Admin/assets/css/font-awesome.css"
	rel="stylesheet" />
<link
	href="/HomeMore/Admin/assets/js/morris/morris-0.4.3.min.css"
	rel="stylesheet" />
<link href="/HomeMore/Admin/assets/css/custom.css"
	rel="stylesheet" />
				...
				<div class="row">
					<div class="col-md-12">
						...
						<!-- <div class="navbar-header">
							...
							<a class="navbar-brand" href="/HomeMore/Admin/account_insert.jsp" 
						...
						<div class="btn btn-success" style="padding: 0px 0px;" >
							<button type="button" class="navbar-toggle"
								...
								<a class="navbar-brand" href="/HomeMore/Admin/account_insert.jsp" style="color: white;text-transform:uppercase;font-size:14px;"> Thêm Tài Khoản</a>
						</div>
					</div>
				</div>
				...
				<div class="row">
					<div class="col-md-12">
						...
						%>style="display:none" <%}%> class="center" style="text-align: center;">
												...
												%>  href="/HomeMore/Admin/account_edit.jsp?action=Update&ID_Account=<%=acc.getID_Account()%>&Password=<%=acc.getPassword()%>&Username=<%=acc.getUsername()%>&Email=<%=acc.getEmail() %>&Fullname=<%=acc.getFull_Name() %>&Address=<%=acc.getAddress() %>&Phone=<%=acc.getSDT() %>"
													class="btn btn-info btn-xs" style="margin-bottom:5px">Sửa</a> 
													
													<a <% String ID_Account = acc.getID_Account();
													if (acc.getID_Account().equals(user.getID_Account())|| acc.getID_Account().equals("AD101") || user.getAccess() == 3 ||user.getAccess() == 4 ) {											
														%>style="display:none" <%}
												...
												%>  href="/HomeMore/AccountServlet_Delete?action=Delete&ID_Account=<%=acc.getID_Account()%>" onclick="return confirm('Bạn chắc chắn muốn xóa tài khoản <%=acc.getUsername()%> chứ ?')"
